...
const Shrutapex = () => {
  ...
  const lastRestartTimeRef = useRef(0); // Track last restart time to prevent loops
      ...
      if (finalTranscript) {
        ...
        // Normal capture - inline the field capture logic
          ...
          if (stepInfo && stepInfo.field === 'symptoms') {
            ...
            console.log('⚠️ Social History uses dropdowns - say NEXT to skip or fill manually');
            // Just log, user says NEXT to skip
          } else if (stepInfo && stepInfo.field === 'vitals') {
            console.log('⚠️ Vitals use structured fields - say NEXT to skip or fill manually');
            // Just log, user says NEXT to skip
  ...
  const extractInformationByTriggers = (text) => {
    ...
    const triggerPatterns = {
      ...
      allergies: [
        /(?:allergies?|allergic to|allergy|adverse reaction):?\s*([a-z\s]+?)(?:\s+and has|\s+and|\.|\;|\,|$)/gi,
        /(?:cannot take|avoid|sensitive to):?\s*([a-z\s]+?)(?:\.|\;|\,|$)/gi
      ],
      socialHistory: [
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 p-4 md:p-8">
      ...
      <div className="max-w-6xl mx-auto">
          ...
          <div className="relative">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                ...
                <div id="field-socialHistory" className={`${guidedFlowStep === 3 && isListening ? 'ring-8 ring-emerald-400/70 rounded-xl p-3 -m-3 bg-emerald-500/15 shadow-2xl shadow-emerald-500/50' : ''} transition-all duration-300`}>
                  <div className="mb-2">
                    <label className="block text-orange-300 font-semibold text-sm uppercase tracking-wide flex items-center gap-2">
                      🏠 Social History 
                      {guidedFlowStep === 3 && isListening && (
                        <span className="ml-2 flex items-center gap-1.5 text-xs bg-emerald-500 text-white px-3 py-1 rounded-full shadow-lg shadow-emerald-500/50">
                          <Mic className="w-3 h-3 animate-pulse" />
                          <span className="font-bold">LISTENING - Fill manually or say "NEXT"</span>
                        </span>
                      )}
                    </label>
                  </div>
                ...
                {/* Allergies - Step 5 */}
                <div id="field-allergies" className={`${guidedFlowStep === 5 && isListening ? 'ring-8 ring-emerald-400/70 rounded-xl p-3 -m-3 bg-emerald-500/15 shadow-2xl shadow-emerald-500/50' : ''} transition-all duration-300`}>
                  <div className="flex items-center justify-between mb-2">
        ...
        <div id="field-diagnosis" className={`relative overflow-hidden bg-gradient-to-br from-slate-800/50 to-slate-900/50 backdrop-blur-xl rounded-2xl shadow-2xl p-8 mb-6 border ${guidedFlowStep === 6 && isListening ? 'border-emerald-400 ring-8 ring-emerald-400/70 shadow-emerald-500/50 shadow-2xl' : 'border-slate-700/50'} transition-all duration-300`}>
          <div className={`absolute inset-0 ${guidedFlowStep === 6 && isListening ? 'bg-gradient-to-br from-emerald-500/15 to-green-500/15' : 'bg-gradient-to-br from-green-500/5 to-emerald-500/5'}`}></div>
          <div className="relative">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-2xl font-bold text-white flex items-center gap-3">
          ...
          <div className="relative">
            ...
            <div className="mb-6 p-6 bg-gradient-to-br from-blue-500/10 to-indigo-500/10 rounded-xl border border-blue-500/30">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
                ...
                {/* Route of Administration - Step 3 */}
                <div className={`${guidedFlowStep === 7 && prescriptionSubStep === 3 ? 'ring-8 ring-emerald-400/70 rounded-xl p-3 -m-3 bg-emerald-500/15 shadow-2xl shadow-emerald-500/50' : ''} transition-all duration-300`}>
                  <label className="block text-blue-200 text-sm font-medium mb-2 flex items-center gap-2">
                    Route of Administration * 
                    {guidedFlowStep === 7 && prescriptionSubStep === 3 && (
                      <span className="flex items-center gap-1.5 text-xs bg-emerald-500 text-white px-3 py-1 rounded-full shadow-lg shadow-emerald-500/50">
                        <Mic className="w-3 h-3" />
                        <span className="font-bold">LISTENING</span>
                      </span>
                ...
                {/* Frequency - Step 4 */}
                <div className={`${guidedFlowStep === 7 && prescriptionSubStep === 4 ? 'ring-8 ring-emerald-400/70 rounded-xl p-3 -m-3 bg-emerald-500/15 shadow-2xl shadow-emerald-500/50' : ''} transition-all duration-300`}>
                  <label className="block text-blue-200 text-sm font-medium mb-2 flex items-center gap-2">
                    Frequency * 
                    {guidedFlowStep === 7 && prescriptionSubStep === 4 && (
                      <span className="flex items-center gap-1.5 text-xs bg-emerald-500 text-white px-3 py-1 rounded-full shadow-lg shadow-emerald-500/50">
                        <Mic className="w-3 h-3" />
                        <span className="font-bold">LISTENING</span>
                      </span>
                ...
                {/* Food Instructions - Step 6 */}
                <div className={`${guidedFlowStep === 7 && prescriptionSubStep === 6 ? 'ring-8 ring-emerald-400/70 rounded-xl p-3 -m-3 bg-emerald-500/15 shadow-2xl shadow-emerald-500/50' : ''} transition-all duration-300`}>
                  <label className="block text-blue-200 text-sm font-medium mb-2 flex items-center gap-2">
                    Food Instructions * 
                    {guidedFlowStep === 7 && prescriptionSubStep === 6 && (
                      <span className="flex items-center gap-1.5 text-xs bg-emerald-500 text-white px-3 py-1 rounded-full shadow-lg shadow-emerald-500/50">
                        <Mic className="w-3 h-3" />
                        <span className="font-bold">LISTENING</span>
                      </span>
        ...
        <div className="relative overflow-hidden bg-gradient-to-br from-slate-800/50 to-slate-900/50 backdrop-blur-xl rounded-2xl shadow-2xl p-8 mb-6 border border-slate-700/50">
          ...
          <div className="relative">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {/* Lab Tests - Step 8 */}
              <div id="field-labTests" className={`${guidedFlowStep === 8 && isListening ? 'ring-8 ring-emerald-400/70 rounded-xl p-3 -m-3 bg-emerald-500/15 shadow-2xl shadow-emerald-500/50' : ''} transition-all duration-300`}>
                <div className="flex items-center justify-between mb-2">
                  <label className="block text-teal-300 font-semibold text-sm uppercase tracking-wide flex items-center gap-2">
                    🔬 Lab Tests 
                    {guidedFlowStep === 8 && isListening && (
                      <span className="ml-2 flex items-center gap-1.5 text-xs bg-emerald-500 text-white px-3 py-1 rounded-full shadow-lg shadow-emerald-500/50">
                        <Mic className="w-3 h-3 animate-pulse" />
                        <span className="font-bold">LISTENING</span>
                      </span>
                  ...
                  <button
                    onClick={() => {
                      ...
                      setCurrentPrompt(GUIDED_STEPS[8].prompt);
                      if (!isListening) toggleListening();
                    }}
              ...
              {/* Advice - Step 9 */}
              <div id="field-advice" className={`${guidedFlowStep === 9 && isListening ? 'ring-8 ring-emerald-400/70 rounded-xl p-3 -m-3 bg-emerald-500/15 shadow-2xl shadow-emerald-500/50' : ''} transition-all duration-300`}>
                <div className="flex items-center justify-between mb-2">
                  <label className="block text-indigo-300 font-semibold text-sm uppercase tracking-wide flex items-center gap-2">
                    💡 Advice & Instructions 
                    {guidedFlowStep === 9 && isListening && (
                      <span className="ml-2 flex items-center gap-1.5 text-xs bg-emerald-500 text-white px-3 py-1 rounded-full shadow-lg shadow-emerald-500/50">
                        <Mic className="w-3 h-3 animate-pulse" />
                        <span className="font-bold">LISTENING</span>
                      </span>
              ...
              {/* Follow-up - Step 11 */}
              <div id="field-followUpInstructions" className={`${guidedFlowStep === 11 && isListening ? 'ring-8 ring-emerald-400/70 rounded-xl p-3 -m-3 bg-emerald-500/15 shadow-2xl shadow-emerald-500/50' : ''} transition-all duration-300`}>
                <div className="flex items-center justify-between mb-2">
                  <label className="block text-blue-300 font-semibold text-sm uppercase tracking-wide flex items-center gap-2">
                    📅 Follow-up 
